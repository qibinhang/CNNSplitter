...
"""
the initial ga.py file is from https://github.com/ahmedfgad/GeneticAlgorithmPython/blob/master/pygad.py
thanks the authors.
some functions in ga.py have been modified in this study.
"""
...
class GA:
    def __init__(self,
                 ...
                 keep_parents=-1,
                 K_tournament=3,
                 explorer_finish_signal=None,
                 recorder_finish_signal=None,
        ...
        self.explorer_finish_signal = explorer_finish_signal
        self.recorder_finish_signal = recorder_finish_signal
                ...
                for index, el in enumerate(gene_space):
                    if type(el) in [list, tuple, range, numpy.ndarray]:
                        if len(el) == 0:
                            ...
                            for val in el:
                                if not (type(val) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32,
                                                      numpy.int64, numpy.float, numpy.float16, numpy.float32,
                                                      numpy.float64]):
                                    raise TypeError(
                                        "All values in the sublists inside the 'gene_space' attribute must be numeric of type int/float but ({val}) of type {typ} found.".format(
                                            val=val, typ=type(val)))
                        self.gene_space_nested = True
                    elif type(el) == type(None):
                        self.gene_space_nested = True
                    elif not (type(el) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32, numpy.int64,
            ...
            if len(gene_space) != num_genes:
                ...
                raise TypeError(
                    "When the parameter 'gene_space' is nested, then its length must be equal to the value passed to the 'num_genes' parameter. Instead, length of gene_space ({len_gene_space}) != num_genes ({len_num_genes})".format(
                        len_gene_space=len(gene_space), len_num_genes=num_genes))
        ...
        if gene_type in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32, numpy.int64, numpy.float,
                         numpy.float16, numpy.float32, numpy.float64]:
            self.gene_type = gene_type
        else:
            self.valid_parameters = False
            raise ValueError(
                "The value passed to the 'gene_type' parameter must be either int or float but ({gene_type}) found.".format(
                    gene_type=gene_type))
        ...
        elif type(crossover_probability) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32, numpy.int64,
                                             numpy.float, numpy.float16, numpy.float32, numpy.float64]:
        ...
        if not (self.mutation_type is None):
            if mutation_probability is None:
                ...
                if type(mutation_probability) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32,
                                                  numpy.int64, numpy.float, numpy.float16, numpy.float32,
                                                  numpy.float64]:
                ...
                if type(mutation_probability) in [list, tuple, numpy.ndarray]:
                    if len(mutation_probability) == 2:
                        for el in mutation_probability:
                            if type(el) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32, numpy.int64,
                                            numpy.float, numpy.float16, numpy.float32, numpy.float64]:
                                if el >= 0 and el <= 1:
                                    pass
                                else:
        ...
        if not (self.mutation_type is None):
            if mutation_num_genes is None:
                ...
                if (mutation_type != "adaptive"):
                    ...
                    elif type(mutation_percent_genes) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32,
                                                          numpy.int64, numpy.float, numpy.float16, numpy.float32,
                                                          numpy.float64]:
                    ...
                    if type(mutation_percent_genes) in [list, tuple, numpy.ndarray]:
                        if len(mutation_percent_genes) == 2:
                            ...
                            for idx, el in enumerate(mutation_percent_genes):
                                if type(el) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32,
                                                numpy.int64, numpy.float, numpy.float16, numpy.float32, numpy.float64]:
                                    if (el <= 0 or el > 100):
                                        self.valid_parameters = False
                                        raise ValueError(
                        ...
                        if self.mutation_probability is None:
                            self.valid_parameters = False
                            raise ValueError(
                                "Unexpected type for the 'mutation_percent_genes' parameter. When mutation_type='adaptive', then list/tuple/numpy.ndarray is expected but ({mutation_percent_genes_value}) of type {mutation_percent_genes_type} found.".format(
                                    mutation_percent_genes_value=mutation_percent_genes,
                                    mutation_percent_genes_type=type(mutation_percent_genes)))
        ...
        # delay_after_gen
        if type(delay_after_gen) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32, numpy.int64,
                                     numpy.float, numpy.float16, numpy.float32, numpy.float64]:
                ...
                for gene_idx in range(self.num_genes):
                    ...
                    if type(curr_gene_space) in [list, tuple, range]:
                        ...
                        self.population[sol_idx, gene_idx] = self.gene_type(numpy.random.uniform(low=low,
                                                                                                 high=high,
                                                                                                 size=1))
                    elif type(curr_gene_space) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32,
                                                   numpy.int64, numpy.float, numpy.float16, numpy.float32,
                                                   numpy.float64]:
                        self.population[sol_idx, gene_idx] = curr_gene_space
        else:
            # Creating the initial population by randomly selecting the genes' values from
            # the values inside the 'gene_space' parameter.
    ...
    def cal_pop_fitness(self, generation):

        """
        load the fitness file which is generated by module_recorder.py
        """
        while True:
            if os.path.exists(self.recorder_finish_signal):
                os.remove(self.recorder_finish_signal)
                break
            else:
                time.sleep(1)

        fitness_file_path = f'{self.save_dir}/gen_{generation}_fitness.pkl'
        with open(fitness_file_path, 'rb') as f:
            fitness = pickle.load(f)

        pop_fitness = fitness[self.target_class]

        return pop_fitness

    def cal_sol_outputs(self):

        """
        return outputs of modules which obtained by solutions.
        """

        sol_outputs = []
        # Calculating the outputs of each solution in the current population.
        for sol_idx, sol in enumerate(tqdm(self.population, desc=f'class_{self.target_class}_sol_outputs', ncols=100)):
            each_sol_output = self.fitness_func(sol, sol_idx)
            sol_outputs.append(each_sol_output)

        return sol_outputs

    def save_sol_outputs(self, sol_outputs, gen_idx):
        with open(f'{self.save_dir}/gen_{gen_idx}_exp_{self.target_class}_outputs.pkl', 'wb') as f:
            pickle.dump(sol_outputs, f)

        with open(self.explorer_finish_signal, 'w') as f:
            f.write('ok')
    ...
    def run(self):
        ...
        self.num_generations += self.start_generation
        for generation in range(self.start_generation, self.num_generations):
            print(f'generation: {generation}')
            print('-'*100)
            # Measuring the fitness of each chromosome in the population.
            sol_outputs = self.cal_sol_outputs()
            self.save_sol_outputs(sol_outputs, generation)

            fitness = self.cal_pop_fitness(generation)
            best_solution, best_solution_fitness, best_match_idx = self.best_solution(pop_fitness=fitness)

            # Appending the fitness value of the best solution in the
            # current generation to the best_solutions_fitness attribute.
            ...
            # If self.crossover_type=None, then no crossover is applied and thus no offspring
            # will be created in the next generations.
            ...
            # If self.mutation_type=None, then no mutation is applied and
            # thus no changes are applied to the offspring created using the crossover operation.
            # The offspring will be used unchanged in the next generation.
            ...
            if self.save_checkpoint:
                with open(f'{self.save_dir}/gen_{generation+1}_exp_{self.target_class}_pop.pkl', 'wb') as f:
        ...
        print('-' * 100)
        sol_outputs = self.cal_sol_outputs()
        self.save_sol_outputs(sol_outputs, self.num_generations)
        print(f'\nFinish.\n')
        ...
        for offspring_idx in range(offspring.shape[0]):
            ...
            for gene_idx in mutation_indices:
                ...
                if self.gene_space_nested:
                    ...
                    if type(curr_gene_space) in [int, float, numpy.int, numpy.int8, numpy.int16, numpy.int32,
                                                 numpy.int64, numpy.float, numpy.float16, numpy.float32, numpy.float64]:
