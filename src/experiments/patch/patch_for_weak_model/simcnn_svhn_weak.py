...
class SimCNNSVHNWeak(nn.Module):
    def __init__(self, num_classes=10, conv_configs=None):
        super(SimCNNSVHNWeak, self).__init__()
        self.num_classes = num_classes
        is_modular = True
        if conv_configs is None:
            conv_configs = [(3, 8), (8, 8)]
            is_modular = False
        ...
        for i, each_conv_config in enumerate(conv_configs):
            ...
            setattr(self, f'conv_{i}', nn.Conv2d(in_channel, out_channel, kernel_size=5))

        self.fc_2 = nn.Linear(conv_configs[-1][-1] * 25, num_classes)
