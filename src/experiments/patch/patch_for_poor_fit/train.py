...
from simcnn_poor_fit import SimCNNPoorFit
from rescnn_poor_fit import ResCNNPoorFit
...
def train(model, train_loader, val_loader, model_save_dir):
    ...
    best_model = None
    optimization = torch.optim.SGD(model.parameters(), lr=lr)
...
def main():
    ...
    for target_class in range(num_classes):
        ...
        model = SimCNNPoorFit(num_classes=num_classes) if model_name == 'simcnn' else ResCNNPoorFit(num_classes=num_classes)
        model = model.to(device)
        print(model)

        model_save_dir = f'{configs.data_dir}/patch/poor/{model_name}_{dataset_name}/target_class_{target_class}'
